...
환경 변수 (ATLASSIAN_API_TOKEN 만 필수, 나머지는 기본값 사용 가능):
    ...
    ATLASSIAN_EMAIL         기본값: nices96@osci.kr
...
BASE_URL       = os.getenv("ATLASSIAN_BASE_URL", "https://sg-cf-migtest1.atlassian.net")
EMAIL          = os.getenv("ATLASSIAN_EMAIL", "nices96@osci.kr")
...
def detect_fix_types(
    ...
    if '"relatedType":"linked"' in rule_json and '"blocks"' in rule_json:
...
def fix_issue_link_branch(node: Any, related_type: str) -> None:
    """relatedType="linked"+linkTypes=["blocks"] → relatedType으로 변환."""
    if isinstance(node, dict):
        if node.get("type") == "jira.issue.related":
            value_obj = node.get("value")
            if isinstance(value_obj, dict):
                link_types = value_obj.get("linkTypes", [])
                if value_obj.get("relatedType") == "linked" and "blocks" in link_types:
                    value_obj["relatedType"] = related_type
                    print(f"  ISSUE_LINK_BRANCH 변환: linked+blocks → {related_type}")
        for v in node.values():
